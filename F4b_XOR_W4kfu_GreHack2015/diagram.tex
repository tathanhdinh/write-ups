...
% };
  ...
  \draw[align=center] (0,-4.5) rectangle node(tran0){transition code} (2.5,-3);
  ...
  \draw[align=center] (5,0) rectangle node(ret1){return address} (7.5,2);
  \draw[] (5,-0.5) rectangle node{entry length} (7.5,0);
  \draw[] (5,-3) rectangle node{$\dots$} (7.5,-0.5);
  \draw[align=center] (5,-4.5) rectangle node(tran1){transition code} (7.5,-3);
  ...
  \draw[] (11,2.5) node{entry $\dots$};
  % \draw[] (12.25,-3.5) node{$\dots$};

  \draw[->,dashed] (tran0) to[out=0,in=180] (ret1);
  \draw[->,dashed] (tran1) to[out=0,in=180] (10,1);
\end{tikzpicture}
