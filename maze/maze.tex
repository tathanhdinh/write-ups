...
There is a problem that the calls to \texttt{sleep} will slow down the tracing process, so we use the standard \texttt{LD\_PRELOAD} trick to bypass this function. We implement an empty \texttt{sleep}, compile it as a shared library and set \texttt{LD\_PRELOAD} to the compiled file (c.f.~\Cref{fig:sleep}).
\begin{listing}
